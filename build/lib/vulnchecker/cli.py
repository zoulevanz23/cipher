...
def main(path, lock_file, output_format, json_flag, output, min_severity, ecosystem, fail_on, do_fix, quiet, no_color, do_init_config):
    ...
    if do_fix and ecosystem == "npm":
        ...
        if fixes:
            console.print("\n[bold green]Fix Suggestions:[/]")
            fix_table = Table(border_style="green")
            fix_table.add_column("Package", style="cyan")
            fix_table.add_column("Current", style="red")
            fix_table.add_column("Recommended", style="green")
            fix_table.add_column("Latest")
            for f in fixes:
                fix_table.add_row(
                    f["package_name"],
                    f["current_version"],
                    f"[green]{f['recommended_version']}[/]",
                    f["latest_version"],
                )
